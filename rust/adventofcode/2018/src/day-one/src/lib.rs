...
fn calibrate(frequency: &mut i32, adjustment: &str) {
    // Recalibrates the device on your wrist given the current frequency
    // (&mut i32) and an adjustment (&str):
    *frequency += adjustment.parse::<i32>().unwrap();
}
...
pub fn part_one() {
    ...
    for adjustment in INPUT.lines() {
        calibrate(&mut frequency, &adjustment);
    }
    println!("Day 01 Part 1: {}", frequency);
}
...
pub fn part_two() {
    ...
    println!("Day 01 Part 2: {}", frequency);
}
