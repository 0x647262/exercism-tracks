/*
 * The power() function of Section 9.6 can be made faster by having is
 * calculate x^n in a different way. We first notice that if n is a power of 2,
 * then x ^ n can be computed  by squaring. For example, x^4 is the square of
 * x^2, so x^4 can be computed using only 2 multiplications instead of 3. As it
 * happens, this technique can be used even when n is not a power of 2. If n is
 * even, we use the formula x^n = (x^(n/2))^2. If n is odd, then
